...
def test_script_that_imports_local_module_is_converted_to_single_file(
    ...
    chk_script_output(
        script_path="script_with_single_local_import/hello", expected_output=b"Hello\n"
    )
...
def test_can_import_module_from_package(chk_script_output) -> None:
    chk_script_output(
        script_path="script_using_module_in_package/hello", expected_output=b"Hello\n"
    )
...
def test_imported_modules_are_transformed(chk_script_output) -> None:
    chk_script_output(
        script_path="imports_in_imported_modules/hello", expected_output=b"Hello\n"
    )


def test_circular_references_dont_cause_stack_overflow(chk_script_output) -> None:
    chk_script_output(
        script_path="circular_reference/hello", expected_output=b"Hello\n"
    )
...
def test_explicit_relative_imports_with_module_name_are_resolved_correctly(
    ...
    chk_script_output(
        script_path="explicit_relative_import/hello", expected_output=b"Hello\n"
    )
...
def test_explicit_relative_imports_with_module_name_in_package_init_are_resolved_correctly(
    ...
    chk_script_output(
        script_path="explicit_relative_import_in_init/hello", expected_output=b"Hello\n"
    )
...
def test_package_init_can_be_used_even_if_not_imported_explicitly(
    ...
    chk_script_output(
        script_path="implicit_init_import/hello", expected_output=b"Hello\n"
    )


def test_value_import_is_detected_when_import_is_renamed(chk_script_output) -> None:
    chk_script_output(
        script_path="import_from_as_value/hello", expected_output=b"Hello\n"
    )


def test_module_import_is_detected_when_import_is_renamed(chk_script_output) -> None:
    chk_script_output(
        script_path="import_from_as_module/hello", expected_output=b"Hello\n"
    )
...
@pytest.mark.skipif(
    sys.platform != "linux", reason="Windows python does not support shebang"
)
...
def test_script_using_multi_import(chk_script_output) -> None:
    chk_script_output(
        script_path="script_using_multi_import/hello",
        expected_output=b"Hello\nGoodbye\n",
    )
...
def test_script_import_class(chk_script_output) -> None:
    chk_script_output(
        script_path="script_import_class/hello", expected_output=b"Hello World\n"
    )
